          ...
          <div>
            <div className="mb-2 flex gap-2">
              {(['journal', 'conference'] as const).map(v => (
                <button key={v} onClick={() => setVenueType(v)} className={`rounded-xl px-4 py-1.5 text-sm font-semibold transition ${venueType === v ? 'bg-primary text-white' : 'border border-border bg-card text-text-muted hover:border-primary hover:text-primary'}`}>
